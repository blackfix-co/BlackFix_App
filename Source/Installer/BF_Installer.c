...
static const wchar_t BF_LEGAL_TEXT[] =
    ...
    L"이 문구는 사용자가 BlackFix App을 이용하면서 앱에 내장된 앨범, 노래, 영상, 쇼츠, 굿즈 정보와 외부 링크를 볼 때 지켜야 할 기준을 설명하기 위한 안내이며, 개별 사건에 대한 법률 자문은 아닙니다.\r\n"
    L"\r\n"
    L"1. 보호되는 저작물의 범위\r\n"
    ...
    L"\r\n"
    L"3. 링크와 미리보기의 구분\r\n"
    L"BlackFix App은 기본적으로 배포자가 미리 넣어 둔 제목, 설명, 날짜, 외부 링크를 보여주고, 링크 버튼을 누르면 YouTube 등 외부 사이트로 이동하는 구조입니다. "
    ...
    L"또한 앱 안의 1분 미리보기 기능에 사용되는 로컬 음원 또는 영상 파일은 배포자가 앱 빌드 또는 배포 과정에서 준비한 자료만 대상으로 하며, 그 파일을 보유하고 재생하는 행위 자체가 적법해야 합니다. 구매, 구독, 스트리밍 시청 권한이 있다고 해서 별도의 파일 복제, 편집, 배포, 앱 내 재생 권한까지 자동으로 허락되는 것은 아닙니다.\r\n"
    L"\r\n"
    L"4. 콘텐츠 추가 및 수정 제한\r\n"
    L"다운로드 받은 BlackFix App 실행 파일은 앨범, 노래, 영상, 쇼츠, 굿즈 데이터를 사용자가 직접 추가하거나 수정하는 기능을 제공하지 않습니다. 콘텐츠 목록은 배포자가 빌드 전에 소스 코드에 포함한 자료로 고정되며, 일반 사용자는 앱 안에서 제목, 설명, 링크, 미리보기 파일, 이미지, 굿즈 정보를 새로 넣을 수 없습니다. "
    L"배포자가 새 자료를 넣어 다시 빌드하는 경우에는 제목, 설명, 링크, 미리보기 파일, 이미지, 아이콘, 굿즈 디자인, 로고, 멤버명 표시가 제3자의 권리를 침해하지 않는지 배포자가 확인해야 합니다. "
    ...
    L"권리자, 플랫폼, 당사자로부터 삭제 요청이나 수정 요청이 오면 배포자는 즉시 문제 콘텐츠를 제거하거나 링크를 수정한 새 버전을 배포해야 하며, 반복 침해가 발생하지 않도록 내장 데이터와 미리보기 파일을 관리해야 합니다.\r\n"
    L"\r\n"
    L"7. 굿즈와 이미지 권리\r\n"
    ...
    L"\r\n"
    L"8. 책임 범위\r\n"
    L"BlackFix App은 콘텐츠 목록을 표시하고 배포자가 지정한 외부 링크 또는 로컬 미리보기 파일을 열 수 있게 하는 도구입니다. "
    L"내장 자료의 적법성, 권리자 허락 여부, 링크 대상의 적법성, 로컬 파일 보유 및 이용 권한, 배포로 인한 책임은 배포자에게 있으며, 사용자는 앱이 제공하는 범위 안에서만 콘텐츠를 이용해야 합니다. "
    L"법령, 판례, 플랫폼 정책은 변경될 수 있으므로 공개 배포, 팬 사이트 운영, 수익화, 광고, 행사 상영, 단체 공유, 굿즈 판매와 연결되는 사용은 권리자 허락 또는 전문가 검토를 받은 뒤 진행해야 합니다.\r\n"
    L"\r\n"
    L"아래 확인란을 선택하면, 사용자는 위 내용을 읽고 이해했으며, 다운로드 받은 실행 파일에서는 콘텐츠를 직접 추가하거나 수정할 수 없고 앱이 제공하는 범위 안에서만 이용해야 함을 확인한 것으로 봅니다.";
